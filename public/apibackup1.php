...
$allowList  = array_filter(array_map('trim', explode(',', cfg('CORS_ALLOW_ORIGINS','*')))); // e.g. "https://convert.viblueink.com,https://app.viblueink.com"
$allowCreds = strtolower(cfg('CORS_ALLOW_CREDENTIALS','true')) === 'true';

/**
 * Decide allowed origin:
 * - If allowlist == ["*"] and NO credentials => "*"
 * - If allowlist == ["*"] and credentials     => echo back the request origin (if present)
 * - Else if origin is in allowlist            => echo that origin
 * - Else no ACAO (will fail CORS on browser)
 */
$allowedOrigin = null;
if ($allowList === ['*']) {
  if ($allowCreds) {
...
// Always vary by Origin so caches don’t poison
header('Vary: Origin');
if ($allowedOrigin) {
...
// Methods and headers (reflect requested headers for preflight)
header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
...
// Default content-type for JSON routes (individual handlers can override)
header('Content-Type: application/json; charset=utf-8');
...
// ENV/.env helper (lets you use OS env or a local .env file)
...
/* ---------- GET /api.php?env=1 (optional debug) ---------- */
if (isset($_GET['env'])){
...
/* ---------- POST /api.php?action=confirm (ASYNC uploader) ---------- */
if (isset($_GET['action']) && $_GET['action']==='confirm') {
  ...
  // quick env check (use cfg() so .env works)
  ...
  // seed upload status and spawn worker
  $sfile = status_path($session);
...
/* ---------- helpers for Spring calls ---------- */
function spring_base(){ $s=rtrim(cfg('SPRING_BASE',''),'/'); if(!$s) throw new RuntimeException('SPRING_BASE not set'); return $s; }
...
function http_post_json($url,$payload){
  ...
  if($resp===false) return [null,"cURL error: $err",0];
  $js=json_decode($resp,true);
  if($js===null) return [null,"Invalid JSON from Spring ($code)",$code];
  return [$js,null,$code];
}
...
/* Build ingest payload from final.json or (questions.json + r2-map.json) */
function build_ingest_payload($session){
  ...
  if(is_file($ff)){
    ...
    elseif(is_array($data)) $items=$data; // array style
  }
  if($items===null){
    // fallback: questions + partial map -> do string replacements
    $q = read_json_file($qf);
    $m = read_json_file($mf) ?: [];
  ...
  // Normalize to {items:[...]} for Spring
  return [[ 'data'=>['items'=>$items] ], null, 200];
}

/* Upsert subject + chapter in Spring, returns [subjectId, chapterId] */
function spring_upsert_subject_chapter($subject,$chapter,$className,$chapterNo){
  [$sr,$se,] = http_post_json(spring_base().'/api/mcq/admin/subjects', ['name'=>$subject,'code'=>null]);
  if($se || empty($sr['id'])) throw new RuntimeException('Subject upsert failed: '.($se?:'no id'));
  [$cr,$ce,] = http_post_json(spring_base().'/api/mcq/admin/chapters', [
    'subjectId'=>(int)$sr['id'],'chapterNo'=>$chapterNo?:null,'name'=>$chapter,'className'=>$className
  ]);
  if($ce || empty($cr['id'])) throw new RuntimeException('Chapter upsert failed: '.($ce?:'no id'));
  return [(int)$sr['id'], (int)$cr['id']];
}

/* Actually call Spring ingest */
function spring_ingest_now($session,$subjectId,$chapterId,$chapterNo,$uploadedBy,$sourceName,$extraMeta=[]){
  ...
  // optionally include meta like exam/class for server-side storage later
  $payload['meta'] = ['examTitle'=>$extraMeta['exam']??null, 'className'=>$extraMeta['class']??null];

  return http_post_json(spring_base().'/api/mcq/ingest', $payload);
}

/* ---------- POST /api.php?action=push (queue ingest; ingest immediately if final ready) ---------- */
if (isset($_GET['action']) && $_GET['action']==='push') {
  ...
  // Save a small ingest request so uploader can finish it later
  ...
  // If final.json already exists, ingest immediately
  ...
  try{
    ...
    if($finalExists){
      ...
      if($err){ out_json(['ok'=>false,'queued'=>false,'error'=>$err,'status'=>$code,'step'=>'ingest'],502); }
      out_json(['ok'=>true,'queued'=>false,'message'=>'Ingested immediately','result'=>$res]);
    } else {
      // no final yet -> uploader will perform ingest at the end
      out_json(['ok'=>true,'queued'=>true,'message'=>'Queued for ingest after upload','status_url'=>'api.php?status='.rawurlencode($session)]);
    }
  } catch(Throwable $e){
    out_json(['ok'=>false,'error'=>$e->getMessage(),'step'=>'upsert-subject/chapter'],502);
  }
}

/* ---------- POST /api.php?action=push_retry (retry ingest only; no re-upload) ---------- */
if (isset($_GET['action']) && $_GET['action']==='push_retry') {
  ...
  // Load details from ingest-request.json (so user doesn’t have to type again)
...
// If we’re here, it’s the DOCX upload (no action or unknown action)
